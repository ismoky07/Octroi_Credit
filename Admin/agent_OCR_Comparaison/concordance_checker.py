...
def normaliser_texte_ocr(texte: str) -> str:
    """
    Normalise le texte pour gérer les variations typiques de l'OCR
    """
    if not texte:
        return ""
...
def normaliser_numero(numero: str) -> str:
    """
    Normalise les numéros en enlevant espaces, tirets, points
    """
    if not numero:
        return ""
...
def comparer_textes_normalises(texte1: str, texte2: str, tolerance: float = 0.8) -> bool:
    """
    Compare deux textes normalisés avec tolérance pour erreurs OCR
    """
    ...
    # Calcul de similarité basique (optionnel pour tolérance OCR)
...
def verifier_concordance_complete(infos_documents: Dict[str, DocumentInfo]) -> Tuple[bool, List[str]]:
    """
    Vérification complète de concordance entre documents
    """
    if len(infos_documents) < 2:
        safe_print("Moins de 2 documents à comparer")
        return True, []
    
    problemes = []
    
    # 1. INFORMATIONS D'IDENTITÉ PERSONNELLE
    ...
    # 6. VÉRIFICATIONS CROISÉES SPÉCIALISÉES
...
def _verifier_identite_personnelle(infos_documents: Dict[str, DocumentInfo]) -> List[str]:
    """Vérifie la cohérence des informations d'identité"""
    problemes = []
    ...
    # Vérifier les prénoms
    if len(prenoms) >= 2:
        prenoms_normalises = {normaliser_texte_ocr(prenom) for _, prenom in prenoms}
        if len(prenoms_normalises) > 1:
            problemes.append(f"Discordance des prénoms: {', '.join(f'{prenom} ({os.path.basename(chemin)})' for chemin, prenom in prenoms)}")
...
def _verifier_identifiants_officiels(infos_documents: Dict[str, DocumentInfo]) -> List[str]:
    """Vérifie la cohérence des identifiants officiels"""
    problemes = []
    ...
    if len(nums_cin) >= 2:
        ...
        if len(cin_normalises) > 1:
            problemes.append(f"Discordance des numéros CIN: {', '.join(f'{num} ({os.path.basename(chemin)})' for chemin, num in nums_cin)}")
    
    # Numéros de sécurité sociale (si disponible)
    ...
    if len(nums_secu) >= 2:
        ...
        if len(secu_normalises) > 1:
            problemes.append(f"Discordance des numéros de sécurité sociale: {', '.join(f'{num} ({os.path.basename(chemin)})' for chemin, num in nums_secu)}")
    
    return problemes

def _verifier_domicile_contact(infos_documents: Dict[str, DocumentInfo]) -> List[str]:
    """Vérifie la cohérence des informations de domicile et contact"""
    problemes = []
    ...
    if len(adresses) >= 2:
        ...
        if len(groupes_adresses) > 1:
            problemes.append(f"Possible discordance des adresses détectée entre {len(groupes_adresses)} groupes différents")
    ...
    if len(telephones) >= 2:
        ...
        if len(tel_normalises) > 1:
            problemes.append(f"Discordance des numéros de téléphone: {', '.join(f'{tel} ({os.path.basename(chemin)})' for chemin, tel in telephones)}")
...
def _verifier_coherence_financiere(infos_documents: Dict[str, DocumentInfo]) -> List[str]:
    ...
    # Vérification salaire vs virements (logique métier spécifique)
    ...
    if salaires and virements:
        # Logique de comparaison salaire/virement à implémenter selon besoins
        pass
    
    return problemes

def _verifier_coherence_temporelle(infos_documents: Dict[str, DocumentInfo]) -> List[str]:
    """Vérifie la cohérence temporelle des documents"""
    problemes = []
    
    # Dates d'émission des documents
    ...
    if len(dates_emission) >= 2:
        # Vérifier que les dates ne sont pas trop éloignées (ex: plus de 6 mois)
        try:
            dates_parsed = []
            ...
            if len(dates_parsed) >= 2:
                ...
                if ecart_max.days > 180:  # Plus de 6 mois
                    problemes.append(f"Écart important entre les dates d'émission des documents: {ecart_max.days} jours")
        
        except Exception as e:
            safe_print(f"Erreur lors de la vérification temporelle: {e}")
    
    return problemes

def _verifier_croisements_specifiques(infos_documents: Dict[str, DocumentInfo]) -> List[str]:
    """Vérifications croisées spécialisées selon le type de document"""
    problemes = []
    
    # Identifier les types de documents disponibles
    types_docs = {info.type_document for info in infos_documents.values()}
    
    # Vérifications spécifiques CIN + Facture électricité
    if 'cin' in types_docs and 'facture_electricite' in types_docs:
        # L'adresse doit correspondre
        # (déjà vérifié dans _verifier_domicile_contact mais on peut ajouter des règles spécifiques)
        pass
    
    # Vérifications spécifiques Bulletin salaire + Relevé bancaire
...
def analyser_concordance_detaillee(infos_documents: Dict[str, DocumentInfo]) -> Dict[str, any]:
    """
    Analyse détaillée de la concordance avec statistiques complètes
    """
    if not infos_documents:
        return {"analyse": "Aucun document à analyser"}
...
def _calculer_score_confiance(stats: Dict, problemes: List[str]) -> float:
    """Calcule un score de confiance basé sur la concordance et la complétude"""
    score_base = 100.0
    
    # Pénalités pour les problèmes détectés
    score_base -= len(problemes) * 15
    
    # Bonus pour la complétude des informations
...
def _generer_recommandations(stats: Dict) -> List[str]:
    ...
    if stats["documents_avec_cin"] == 0:
        recommandations.append("Aucun numéro CIN détecté - vérifier la qualité de l'OCR sur la CIN")
    
    if stats["documents_avec_nom"] < 2:
        recommandations.append("Peu de documents contiennent le nom - améliorer l'extraction")
    
    if not stats["concordance_globale"]:
        recommandations.append("Discordances détectées - vérification manuelle recommandée")
