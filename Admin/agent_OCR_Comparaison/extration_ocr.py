...
from utilsAgentOCR import safe_print, safe_text_handling
...
def construire_prompt_ocr() -> dict:
    """Construit le prompt OCR optimisé"""
    return {
        "role": "user",
        "content": [
...
- BULLETIN_SALAIRE
- RELEVE_BANCAIRE
- JUSTIFICATIF_DOMICILE (autre que facture électricité)
- AUTRE (spécifie lequel)

**ÉTAPE 2 - EXTRACTION CIBLÉE PAR TYPE**
...
OBSERVATIONS:
- [Notes sur la qualité, problèmes détectés]
```

**RÈGLES IMPORTANTES :**
1. Privilégie la précision sur la quantité - mieux vaut marquer ILLISIBLE que deviner
2. Normalise les formats de date en JJ/MM/AAAA
3. Pour les montants, indique l'unité (DH, MAD)
4. Pour les adresses, extrais l'adresse complète
5. Attention aux variations d'écriture (manuscrit vs imprimé)
...
def construire_prompt_recuperation() -> dict:
    """Prompt spécialisé pour documents de mauvaise qualité"""
    return {
        "role": "user", 
        "content": [
            {"type": "text", "text": """Ce document semble de mauvaise qualité. Mode récupération activé :

1. Identifie les zones de texte les plus lisibles
2. Concentre-toi sur les informations critiques : nom, prénom, numéros
...
OBSERVATIONS:
- Document de très mauvaise qualité
...
def extraire_infos_documents(client, chemins_images: list) -> Dict[str, dict]:
    """
    Extrait les informations des documents avec validation de qualité
    Retourne un dictionnaire avec analyses complètes
    """
    ...
    for chemin in chemins_images:
        try:
            ...
            if not base64_image:
                ...
                resultats[chemin] = {
                    "extraction_brute": "ERREUR: Problème d'encodage",
                    "qualite": "ERREUR",
                    "parsed_info": None
                }
                continue
            ...
            safe_print(f"Erreur générale pour {chemin}: {str(e)}")
            resultats[chemin] = {
...
def _extraire_avec_gestion_qualite(client, base64_image: str, chemin: str) -> dict:
    """Extrait avec gestion intelligente de la qualité"""
    
    # Première tentative avec prompt normal
    try:
        ...
        # Si qualité très faible, essayer le mode récupération
        if qualite["niveau"] == "FAIBLE":
            safe_print(f"Qualité faible détectée pour {chemin}, tentative mode récupération...")
...
def _tentative_recuperation(client, base64_image: str, extraction_normale: str, parsed_normal: dict) -> dict:
    """Tentative de récupération pour documents difficiles"""
    try:
        ...
        safe_print(f"Erreur mode récupération: {e}")
        return {
            "extraction_brute": extraction_normale,
...
def parser_informations_ameliore(texte_document: str) -> dict:
    """
    Parse le nouveau format structuré de réponse
    """
    try:
        ...
        safe_print(f"Erreur parsing amélioré: {str(e)}")
        return {
...
def convertir_vers_document_info(parsed_data: dict) -> DocumentInfo:
    """Convertit le dictionnaire parsé vers DocumentInfo"""
    try:
        ...
        # Informations supplémentaires dans autres_infos
...
def evaluer_qualite_extraction(parsed_result: dict) -> dict:
    ...
    # Pénalités selon la qualité d'image
    qualite_image = parsed_result.get("qualite_image", "INCONNUE").upper()
    if qualite_image == "FAIBLE":
        score -= 30
        recommandations.append("Améliorer la qualité de l'image")
    elif qualite_image == "MOYENNE":
        score -= 15
    
    # Pénalités selon la confiance de classification
    confiance = parsed_result.get("confiance_classification", "FAIBLE").upper()
    if confiance == "FAIBLE":
        score -= 25
        recommandations.append("Vérifier le type de document")
    elif confiance == "MOYENNE":
        score -= 10
    
    # Compter les champs problématiques
    ...
    if champs_problematiques > 0:
        ...
        recommandations.append(f"{champs_problematiques} champ(s) problématique(s)")
    
    # Vérifier la complétude des informations essentielles
...
def _fusionner_extractions(normale: dict, recuperation: dict) -> dict:
    ...
    for cle, valeur_recup in info_recuperation.items():
        ...
        # Si la valeur normale est problématique et la récupération est claire
...
def _creer_document_info_erreur(message_erreur: str) -> DocumentInfo:
    """Crée un DocumentInfo pour les cas d'erreur"""
    return DocumentInfo(
        type_document="ERREUR",
...
def analyser_nom_fichier_ameliore(chemin_fichier: str) -> DocumentInfo:
    ...
    # Extraction améliorée d'informations du nom
    # Patterns pour nom/prénom
    patterns_nom = [
...
def _generer_resume_extraction(resultats_ocr: Dict) -> Dict:
    """Génère un résumé de l'extraction OCR uniquement"""
...
def _generer_recommandations_extraction(resultats_ocr: Dict) -> List[str]:
    """Génère des recommandations spécifiques à l'extraction"""
    recommandations = []
    ...
    if docs_faible_qualite:
        recommandations.append(f"{len(docs_faible_qualite)} document(s) de faible qualité détecté(s)")
        recommandations.append("Conseil: Améliorer l'éclairage et la résolution des images")
    ...
    if docs_erreur:
        ...
        recommandations.append("Conseil: Vérifier le format et l'accessibilité des fichiers")
