...
class PDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 10, 'Récapitulatif de demande de crédit à la consommation', 0, 1, 'C')
        self.ln(10)
...
def generer_pdf_recapitulatif(donnees):
    """
    Génère un PDF récapitulatif de la demande de crédit à la consommation
    ...
    # Informations sur le projet (spécifique au crédit à la consommation)
    pdf.set_font('Arial', 'B', 12)
    pdf.cell(0, 10, 'Projet à financer', 0, 1)
    pdf.set_font('Arial', '', 10)
    
    pdf.cell(50, 7, 'Type de projet:', 0, 0)
    pdf.cell(0, 7, donnees["type_projet"], 0, 1)
    
    pdf.cell(50, 7, 'Description:', 0, 0)
    pdf.multi_cell(0, 7, donnees["description_projet"], 0)
    ...
    pdf.cell(50, 7, 'Montant du crédit:', 0, 0)
    pdf.cell(0, 7, f"{donnees['montant']} DH", 0, 1)
    
    pdf.cell(50, 7, 'Durée:', 0, 0)
    pdf.cell(0, 7, f"{donnees['duree']} mois ({donnees['duree']//12} an{'s' if donnees['duree']//12 > 1 else ''})", 0, 1)
    ...
    if "date_premier_remboursement" in donnees:
        pdf.cell(50, 7, 'Premier remboursement:', 0, 0)
        pdf.cell(0, 7, str(donnees["date_premier_remboursement"]), 0, 1)
    ...
    pdf.multi_cell(0, 5, """Note: Ce document est un récapitulatif de votre demande de crédit à la consommation. 
    Il ne constitue pas une offre de crédit définitive. Votre demande sera étudiée par notre service 
    financier et une réponse vous sera communiquée dans les 24-48h ouvrées suivant la réception de votre dossier complet.""")
