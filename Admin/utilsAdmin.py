"""
admin/utilsAdmin.py - Fonctions utilitaires pour l'interface d'administration
VERSION CORRIGÉE avec le bon chemin
"""
import os
import json
...
# CHEMIN CORRIGÉ - Utilisez votre chemin absolu
CHEMIN_BASE_DONNEES = r"C:\Users\HP\Desktop\Octroi-Credit\Admin\demandes_clients"
...
def charger_toutes_demandes() -> List[Dict]:
    ...
    for type_credit, dossier in types_credit.items():
        chemin_base = os.path.join(CHEMIN_BASE_DONNEES, dossier)
...
def obtenir_chemin_dossier(demande: Dict, type_credit: str) -> Optional[str]:
    ...
    chemin_base = os.path.join(CHEMIN_BASE_DONNEES, type_credit)
